...
int Controlador_audio_SDL::Canal_audio_real::volumen_general=0;
...
float Controlador_audio_SDL::Canal_audio_real::calcular_volumen(int v, int vg)
{
	float m=(float)vg / (float)VOLUMEN_GENERAL_INICIO;
	float vol=(float) v * m;
	return vol;
}
...
{
	float vol=calcular_volumen(v, volumen_general);
	Mix_Volume(indice, vol);
}
...
{
	volumen_sonidos_general=p_vol;
	Canal_audio_real::establecer_volumen_general(volumen_sonidos_general);
	for(Canal_audio_real& c : canales) c.establecer_volumen(c.acc_volumen());
}
...
void Controlador_audio_SDL::establecer_volumen(int p_vol, int p_canal)	//p_vol de 0 a 128.
{
	if(p_canal==-1)
	{
		establecer_volumen_general(p_vol);
//		volumen_sonidos_general=p_vol;
//		Mix_Volume(p_canal, p_vol);
	}
	else
	{
		try
