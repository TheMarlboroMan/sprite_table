...
void callback_fin_reproduccion(int p_canal);	//F-f-f-forward!.
...
class Controlador_audio_SDL
{
	...
	static const int VOLUMEN_GENERAL_INICIO=128;

	int volumen_sonidos_general;
	int ratio;
...
//hayamos definido la clase.
	...
	unsigned int acc_volumen_general_sonido() const {return volumen_sonidos_general;}
	...
	class Canal_audio_real
	{
		...
		static float calcular_volumen(int v, int vg); //Donde v es volumen y vg es volumen general.
		...
		int iniciar_fade_out(int p_ms); //p_ms son los milisegundos del fadeout.
		...
		static void establecer_volumen_general(int val) {volumen_general=val;}
		...
		Canal_audio_real(int i):
			indice(i), repeticiones(0), volumen(128), 
			reproduciendo(false), ocupado(false), 
			monitoreado(false), pausado(false), 			 
	...
	friend void callback_fin_reproduccion(int p_canal);
...
class Canal_audio
{
	...
	//void recibir_callback_finalizacion() {canal.reci bir_callback_finalizacion();}
